---
title: "4. Calculate bioclimatic variables"
...
```{r}
tmax <- dsclimtools::read_dsclim( "../Output", "tasmax", start, end, calendar_dates = TRUE, proxy = FALSE)
tmin <- dsclimtools::read_dsclim( "../Output", "tasmin", start, end, calendar_dates = TRUE, proxy = FALSE) 
t <- dsclimtools::read_dsclim( "../Output", "tas", start, end, calendar_dates = TRUE, proxy = FALSE) 
prec <- dsclimtools::read_dsclim( "../Output", "pr", start, end, calendar_dates = TRUE, proxy = FALSE)
...
We prepared presence and background data for running an example of species distribution model. We extracted the environmental values of the current bioclimatic variables calculated as done earlier.  

```{r}
presences <- read.csv("filtered_occurrences.csv") %>% select(-c(1:2))
set.seed(1)
background <- randomPoints(topo14_mask, 400)

Present_bioclim <- stack("../Output/TraCE21ka/bioclim/1140_bio.tif")
topo14_mask <- raster("topo14_mask.tif")

presvals <- extract(Present_bioclim, presences, na.rm = TRUE)
presvals <-  presvals[!rowSums(is.na(presvals)),]

absvals <- extract(Present_bioclim, background,  na.rm = TRUE)
...
Here we calibrated a Generalized Linear Model for our presence/background dataset. Further, we projected the model over our selected period to exemplify the power of our dataset in biogeografical history studies. 
...
Finally, we represented graphically the fluctuations of the model along the selected period.
...
![Figure X. Animation](figures/a4/glm_timeseries.gif)
